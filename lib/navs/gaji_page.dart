...
class _GajiPageState extends State<GajiPage> {
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("Gaji ${DateFormat('MMMM yyyy', 'id_ID').format(_bulan)}"), backgroundColor: Colors.brown.shade800, foregroundColor: Colors.white,
        actions: [IconButton(icon: const Icon(Icons.calendar_month), onPressed: () async { final p=await showDatePicker(context: context, initialDate: _bulan, firstDate: DateTime(2023), lastDate: DateTime(2030)); if(p!=null) setState(()=>_bulan=p); }), IconButton(icon: const Icon(Icons.refresh), onPressed: () async { await db.prosesHitungGajiMingguan(); setState((){}); })]),
      body: Column(children: [
        FutureBuilder<int>(future: db.getTotalGajianSemua(), builder: (c,s)=>Container(width: double.infinity, padding: const EdgeInsets.all(16), color: Colors.orange.shade50, child: Column(children: [
          const Text("TOTAL GAJIAN SEMUA KARYAWAN (Responsive Filter)", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
            ...
            return FutureBuilder<KaryawanData?>(future: (db.select(db.karyawan)..where((t)=>t.id.equals(g.karyawanId))).getSingleOrNull(), builder: (c,kar){
              return FutureBuilder<List<AbsensiData>>(future: db.select(db.absensi).get().then((all)=> all.where((a)=> a.karyawanId==g.karyawanId && a.jamMasuk.isAfter(g.mingguMulai.subtract(const Duration(seconds:1))) && a.jamMasuk.isBefore(g.mingguSelesai.add(const Duration(seconds:1)))).toList()), builder: (c,absSnap){
                ...
                return FutureBuilder<double>(future: db.getRataBintangMingguan(g.karyawanId, g.mingguMulai, g.mingguSelesai), builder: (c,bintangSnap){
                  ...
                  final akumulasiBintangHari = list.length>0? rataBintang : 0;
                  // hitung bonus otomatis buat display
                  int bonusAuto = 0;
                  ...
                  return Card(color: _warnaStatus(g.statusBayar), margin: const EdgeInsets.symmetric(horizontal:12, vertical:6), child: ListTile(
                    ...
                    subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Row(children: [
                        ...
                        Text(" Rata Bintang: ${rataBintang.toStringAsFixed(1)} ⭐ | Auto Bonus: Rp ${fmt.format(bonusAuto)}", style: const TextStyle(fontSize:11, fontWeight: FontWeight.bold)),
                      ]),
                    ...
                    trailing: PopupMenuButton(onSelected: (v) async {
                      if(v=='bonus'){
                        ...
                        showDialog(context: context, builder: (_)=>AlertDialog(
                          title: Text("Input Bonus - Bintang ${rataBintang.toStringAsFixed(1)} (Auto Rp ${fmt.format(bonusAuto)})"),
                          ...
                          actions: [
                            TextButton(onPressed: () async { await db.inputBonusMingguan(g.id, bonusAuto); if(context.mounted) Navigator.pop(context); }, child: Text("PAKAI AUTO Rp ${fmt.format(bonusAuto)}")),
                      ...
                      PopupMenuItem(value: 'bonus', child: Text("💰 Input Bonus (Bintang ${rataBintang.toStringAsFixed(1)})")),
