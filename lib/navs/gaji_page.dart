...
class _GajiPageState extends State<GajiPage> {
  ...
  @override Widget build(BuildContext context){
    // HAPUS Scaffold - ini penyebab blank
    return Column(children: [
      Container(width: double.infinity, padding: const EdgeInsets.all(12), color: Colors.orange.shade50, child: Column(children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text("Gaji ${DateFormat('MMMM yyyy','id_ID').format(_bulan)}", style: const TextStyle(fontWeight: FontWeight.bold)),
          Row(children: [
        ...
        FutureBuilder<int>(future: db.getTotalGajianSemua(), builder: (c,s)=> Text("TOTAL Rp ${fmt.format(s.data??0)}", style: TextStyle(fontSize:22, fontWeight: FontWeight.bold, color: Colors.brown.shade800))),
        const Text("Bonus >=4.5=15% >=4.0=10% >=3.5=5%", style: TextStyle(fontSize:9)),
      ])),
      Expanded(child: StreamBuilder<List<GajiMingguanData>>(
          ...
          return ListView.builder(itemCount: filtered.length, itemBuilder: (_,i){
            ...
            return FutureBuilder<KaryawanData?>(future: (db.select(db.karyawan)..where((t)=>t.id.equals(g.karyawanId))).getSingleOrNull(), builder: (c,kar){
              return FutureBuilder<List<AbsensiData>>(future: db.select(db.absensi).get().then((all)=> all.where((a)=> a.karyawanId==g.karyawanId && a.jamMasuk.isAfter(g.mingguMulai.subtract(const Duration(seconds:1))) && a.jamMasuk.isBefore(g.mingguSelesai.add(const Duration(seconds:1)))).toList()), builder: (c,absSnap){
                ...
                return FutureBuilder<double>(future: db.getRataBintangMingguan(g.karyawanId, g.mingguMulai, g.mingguSelesai), builder: (c,bintangSnap){
                  final rata=bintangSnap.data??0;
                  int bonusAuto=0; if(rata>=4.5) bonusAuto=(g.totalGajiPokok*0.15).toInt(); else if(rata>=4.0) bonusAuto=(g.totalGajiPokok*0.10).toInt(); else if(rata>=3.5) bonusAuto=(g.totalGajiPokok*0.05).toInt();
                  return Card(color: _warnaStatus(g.statusBayar), margin: const EdgeInsets.symmetric(horizontal:12, vertical:6), child: ListTile(
                    ...
                    subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Row(children: [const Icon(Icons.star, size:14, color: Colors.amber), Text(" Rata ${rata.toStringAsFixed(1)} ⭐ Auto Rp ${fmt.format(bonusAuto)}", style: const TextStyle(fontSize:11, fontWeight: FontWeight.bold))]),
                    ...
                    trailing: PopupMenuButton(onSelected: (v) async {
                      if(v=='bonus'){ var ctrl=TextEditingController(text: g.bonusMingguan.toString()); if(!mounted) return; showDialog(context: context, builder: (_)=> AlertDialog(title: Text("Bonus Bintang ${rata.toStringAsFixed(1)} Auto Rp ${fmt.format(bonusAuto)}"), content: TextField(controller: ctrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText:"Bonus Rp", border: OutlineInputBorder())), actions: [TextButton(onPressed: () async { await db.inputBonusMingguan(g.id, bonusAuto); if(context.mounted) Navigator.pop(context); }, child: Text("PAKAI AUTO ${fmt.format(bonusAuto)}")), TextButton(onPressed: () async { await db.inputBonusMingguan(g.id, int.tryParse(ctrl.text)??0); if(context.mounted) Navigator.pop(context); }, child: const Text("SIMPAN MANUAL"))])); }
                      else if(v=='cetak'){ final karyawan=kar.data; if(karyawan==null) return; final kategori=await (db.select(db.kategoriKaryawan)..where((t)=>t.id.equals(karyawan.kategoriId))).getSingleOrNull(); if(kategori==null) return; await ExportPdfTBSlametJaya.exportSlipGajiMingguan(karyawan: karyawan, kategori: kategori, gaji: g, absensiMingguIni: list, rataBintang: rata); }
                      else { await db.tandaiBayar(g.id, v.toString()); }
                    }, itemBuilder: (_)=>[PopupMenuItem(value:'bonus', child: Text("💰 Bonus ${rata.toStringAsFixed(1)}")), const PopupMenuItem(value:'BELUM', child: Text("🔴 Belum")), const PopupMenuItem(value:'MINGGU1', child: Text("🟡 1 Minggu")), const PopupMenuItem(value:'MINGGU2', child: Text("🟢 Lunas")), const PopupMenuItem(value:'cetak', child: Text("🖨️ Cetak Slip"))]),
                  ));
                });
              });
            });
