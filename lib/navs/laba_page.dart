...
class _LabaPageState extends State<LabaPage> {
  ...
  int totalOmset=0, totalBebanOps=0, totalTambahan=0, totalBebanGaji=0, totalPemasukan=0, totalBeban=0, labaKotor=0, labaBersih=0, totalAkumGaji=0;

  @override void initState(){ super.initState(); _init(); }
  Future<void> _init() async {
    allKaryawan = await db.select(db.karyawan).get();
    allKategori = await db.select(db.kategoriKaryawan).get();
    final allAbs = await db.select(db.absensi).get()..sort((a,b)=> a.jamMasuk.compareTo(b.jamMasuk));
    ...
    await _hitungAuto(); if(mounted) setState((){});
  }
  ...
  Future<void> _pilihPeriode() async {
    ...
    if(pilih=='Akumulasi Awal - Continue'){
      final all = await db.select(db.absensi).get()..sort((a,b)=> a.jamMasuk.compareTo(b.jamMasuk));
  ...
  @override Widget build(BuildContext context){
    // TANPA Scaffold - biar gak blank di dalam Main
    return SingleChildScrollView(padding: const EdgeInsets.all(12), child: Column(children: [
      Card(elevation:3, child: Padding(padding: const EdgeInsets.all(12), child: Column(children: [
        ...
        const SizedBox(height:8),
        ListTile(title: Text(filterAkumulasi), subtitle: Text(periodeMulai==null?'Pilih periode':'${DateFormat('dd MMM yy').format(periodeMulai!)} - ${DateFormat('dd MMM yy').format(periodeSelesai!)}'), trailing: const Icon(Icons.calendar_month), onTap: _pilihPeriode),
      ]))),
