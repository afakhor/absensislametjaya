...
class _LabaPageState extends State<LabaPage> {
  ...
  @override Widget build(BuildContext context) {
    ...
    int totalP=omset+pl; int totalB=bo+bl+bg; int lk=omset-bo; int lb=totalP-totalB;
    return Scaffold(
      appBar: AppBar(title: const Text("Laba & Beban Gaji Continue"), backgroundColor: Colors.brown.shade800, foregroundColor: Colors.white),
      body: SingleChildScrollView(padding: const EdgeInsets.all(12), child: Column(children: [
        Card(color: Colors.brown.shade50, child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          StreamBuilder<List<KaryawanData>>(stream: db.watchKaryawan(), builder: (c,s){ final list=s.data??[]; return DropdownButton<int?>(isExpanded: true, value: filterKaryawanId, hint: const Text("Filter: Per ID Karyawan"), items: [const DropdownMenuItem(value: null, child: Text("Semua Karyawan")),...list.map((e)=>DropdownMenuItem(value: e.id, child: Text("ID:${e.id} - ${e.nama}")))], onChanged: (v){ setState(()=>filterKaryawanId=v); _hitungAkumulasi(); }); }),
          StreamBuilder<List<KategoriKaryawanData>>(stream: db.watchKategori(), builder: (c,s){ final list=s.data??[]; return DropdownButton<int?>(isExpanded: true, value: filterKategoriId, hint: const Text("Filter: Per Kategori"), items: [const DropdownMenuItem(value: null, child: Text("Semua Kategori")),...list.map((e)=>DropdownMenuItem(value: e.id, child: Text(e.namaKategori)))], onChanged: (v){ setState(()=>filterKategoriId=v); _hitungAkumulasi(); }); }),
          DropdownButton<String>(isExpanded: true, value: filterWaktu, items: const [DropdownMenuItem(value: 'CONTINUE', child: Text("Akumulasi Awal - Continue")), DropdownMenuItem(value: 'MINGGU', child: Text("Per Minggu")), DropdownMenuItem(value: '2MINGGU', child: Text("Per 2 Minggu")), DropdownMenuItem(value: 'BULAN', child: Text("Per Bulan")), DropdownMenuItem(value: 'RANGE', child: Text("Per Range Tanggal")), DropdownMenuItem(value: 'TANGGAL_TERTENTU', child: Text("Tgl Tertentu Sampai Continue"))], onChanged: (v) async { setState(()=>filterWaktu=v!); if(v=='RANGE' || v=='TANGGAL_TERTENTU'){ final p=await showDateRangePicker(context: context, firstDate: DateTime(2023), lastDate: DateTime.now(), initialDateRange: filterMulai!=null&&filterSelesai!=null?DateTimeRange(start: filterMulai!, end: filterSelesai!):null); if(p!=null){ filterMulai=p.start; filterSelesai=p.end; } } _hitungAkumulasi(); }),
          const Divider(),
        ...
        const SizedBox(height:8),
        Container(padding: const EdgeInsets.all(12), color: Colors.orange.shade50, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("Total Pemasukan: Rp ${fmt.format(totalP)}"), Text("Total Beban: Rp ${fmt.format(totalB)}"), Text("Laba Kotor: Rp ${fmt.format(lk)}", style: const TextStyle(fontWeight: FontWeight.bold)), Text("Laba Bersih: Rp ${fmt.format(lb)}", style: TextStyle(fontWeight: FontWeight.bold, color: lb>=0?Colors.green:Colors.red))])),
        const SizedBox(height:8),
        SizedBox(width: double.infinity, child: ElevatedButton(onPressed: _simpan, style: ElevatedButton.styleFrom(backgroundColor: Colors.brown.shade800, foregroundColor: Colors.white), child: Text(editingId==null?"SIMPAN SIMULASI":"UPDATE"))),
      ])),
    );
  }
}