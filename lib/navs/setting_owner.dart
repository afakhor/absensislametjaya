...
import '../dbases/absen_db.dart'; // TAMBAH INI BIAR watchAbsensiHari & watchKaryawan KEDETEK
...
class _SettingOwnerPageState extends State<SettingOwnerPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Setting Owner - TB Slamet Jaya"), backgroundColor: Colors.brown.shade800, foregroundColor: Colors.white),
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            builder: (context, snap) {
              ...
              return Autocomplete<String>(
                ...
                onSelected: (s){
                  ...
                  setState((){ kategoriTerpilih=kat; katNamaC.text=kat.namaKategori; katTarifC.text=kat.tarifPerHari.toString(); });
                },
                fieldViewBuilder: (ctx,ctrl,fn,_) => TextField(
                  ...
                  onChanged: (v){
                    ...
                    if(found.isNotEmpty){ setState((){ kategoriTerpilih=found.first; katTarifC.text=found.first.tarifPerHari.toString(); }); }
                  }
                )
              );
            }
          ...
          const SizedBox(height: 8),
          TextField(controller: katTarifC, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: "Tarif/Hari (Rp)", border: OutlineInputBorder(), prefixText: "Rp ")),
          const SizedBox(height: 8),
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
              icon: const Icon(Icons.save),
              label: const Text("Tambah / Update Kategori"),
              ...
              onPressed: () async {
                ...
                final ex = await (db.select(db.kategoriKaryawan)..where((t)=>t.namaKategori.equals(katNamaC.text))).getSingleOrNull();
                if(ex!=null){
                  await (db.update(db.kategoriKaryawan)..where((t)=>t.id.equals(ex.id))).write(KategoriKaryawanCompanion(tarifPerHari: drift.Value(int.parse(katTarifC.text))));
                } else {
                  await db.into(db.kategoriKaryawan).insert(KategoriKaryawanCompanion.insert(namaKategori: katNamaC.text, tarifPerHari: int.parse(katTarifC.text)));
                }
          ...
          StreamBuilder<List<KategoriKaryawanData>>(
            ...
            builder: (c,snap){
              ...
              if(snap.data!.isEmpty) return const Text("Buat kategori dulu");
              return Column(children: snap.data!.map((kat)=> Card(color: kategoriTerpilih?.id==kat.id? Colors.orange.shade50:null, child: ListTile(leading: const Icon(Icons.work), title: Text(kat.namaKategori), subtitle: Text("Rp ${formatRp.format(kat.tarifPerHari)}/hari"), trailing: kategoriTerpilih?.id==kat.id? const Icon(Icons.check_circle,color: Colors.green) : ElevatedButton(child: const Text("Pilih"), onPressed: (){ setState((){ kategoriTerpilih=kat; katNamaC.text=kat.namaKategori; katTarifC.text=kat.tarifPerHari.toString(); }); })))).toList());
            }
          ),
          const SizedBox(height: 8),
          ...
          StreamBuilder<List<KaryawanData>>(
            ...
            builder: (c,snap){
              ...
              return Column(children: snap.data!.map((k){
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        leading: GestureDetector(
                          onTap: (){ if(k.fotoPath==null) return; showDialog(context: context, builder: (_)=> Dialog(child: Column(mainAxisSize: MainAxisSize.min, children: [Image.file(File(k.fotoPath!)), Padding(padding: const EdgeInsets.all(8), child: Text(k.nama, style: const TextStyle(fontWeight: FontWeight.bold))), TextButton(onPressed: ()=> Navigator.pop(context), child: const Text("Tutup"))]))); },
                          child: k.fotoPath!=null? ClipOval(child: Image.file(File(k.fotoPath!), width:50,height:50,fit:BoxFit.cover)) : CircleAvatar(child: Text(k.nama.isNotEmpty? k.nama[0] : '?'))
                        ),
                        title: Text(k.nama, style: const TextStyle(fontWeight: FontWeight.bold)),
                        subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text("ID:${k.id} - Kategori: ${kat?.namaKategori?? 'Terhapus'}"),
                          Text("Gaji: Rp ${kat!=null? formatRp.format(kat.tarifPerHari) : '0'}/hari", style: TextStyle(color: Colors.green.shade700, fontWeight: FontWeight.bold))
                        ]),
