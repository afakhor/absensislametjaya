  ...
  @override Widget build(BuildContext context) {
    return Scaffold(body: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      StreamBuilder<List<KategoriKaryawanData>>(stream: db.watchKategori(), builder: (context, snap) {
        ...
        return Autocomplete<String>(optionsBuilder: (val) { if (val.text=='') return namaList; return namaList.where((e) => e.toLowerCase().contains(val.text.toLowerCase())); }, onSelected: (s){ final kat = listKat.firstWhere((e) => e.namaKategori==s); setState((){ kategoriTerpilih=kat; katNamaC.text=kat.namaKategori; katTarifC.text=kat.tarifPerHari.toString(); }); }, fieldViewBuilder: (ctx,ctrl,fn,_) => TextField(controller: ctrl, focusNode: fn, decoration: const InputDecoration(labelText: "Nama Kategori", border: OutlineInputBorder(), suffixIcon: Icon(Icons.history)), onChanged: (v){ katNamaC.text=v; final found=listKat.where((e)=>e.namaKategori.toLowerCase()==v.toLowerCase()).toList(); if(found.isNotEmpty){ setState((){ kategoriTerpilih=found.first; katTarifC.text=found.first.tarifPerHari.toString(); }); } }));
      }),
      const SizedBox(height: 8),
      TextField(controller: katTarifC, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: "Tarif/Hari (auto keisi)", border: OutlineInputBorder())),
      ...
      StreamBuilder<List<KategoriKaryawanData>>(stream: db.watchKategori(), builder: (c,snap){ if(!snap.hasData) return const CircularProgressIndicator(); if(snap.data!.isEmpty) return const Text("Buat kategori dulu"); return Column(children: snap.data!.map((kat)=> Card(color: kategoriTerpilih?.id==kat.id? Colors.orange.shade50:null, child: ListTile(leading: const Icon(Icons.work), title: Text(kat.namaKategori), subtitle: Text("Rp ${formatRp.format(kat.tarifPerHari)}/hari"), trailing: kategoriTerpilih?.id==kat.id? const Icon(Icons.check_circle,color: Colors.green) : ElevatedButton(child: const Text("Pilih"), onPressed: (){ setState((){ kategoriTerpilih=kat; katNamaC.text=kat.namaKategori; katTarifC.text=kat.tarifPerHari.toString(); }); })))).toList()); }),
      const SizedBox(height: 8),
      ...
      StreamBuilder<List<KaryawanData>>(stream: db.watchKaryawan(), builder: (c,snap){ if(!snap.hasData) return const CircularProgressIndicator(); if(snap.data!.isEmpty) return const Text("Belum ada karyawan"); return Column(children: snap.data!.map((k){ return FutureBuilder<KategoriKaryawanData?>(future: (db.select(db.kategoriKaryawan)..where((t)=>t.id.equals(k.kategoriId))).getSingleOrNull(), builder: (c,katSnap){ final kat=katSnap.data; return Card(elevation:3, child: ListTile(leading: k.fotoPath!=null? ClipOval(child: Image.file(File(k.fotoPath!), width:50,height:50,fit:BoxFit.cover)) : CircleAvatar(child: Text(k.nama.isNotEmpty? k.nama[0] : '?')), title: Text(k.nama, style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("Kategori: ${kat?.namaKategori?? 'Terhapus'}"), Text("Gaji: Rp ${kat!=null? formatRp.format(kat.tarifPerHari) : '0'}/hari", style: TextStyle(color: Colors.green.shade700, fontWeight: FontWeight.bold))]), trailing: IconButton(icon: const Icon(Icons.delete,color: Colors.red), onPressed: () async { await (db.delete(db.karyawan)..where((t)=>t.id.equals(k.id))).go(); })) ); }); }).toList()); })
    ])));
  }
}