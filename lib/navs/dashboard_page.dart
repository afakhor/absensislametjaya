...
class _DashboardPageState extends State<DashboardPage> {
    ...
    return Row(
      ...
      children: List.generate(5, (i) {
        ...
        return InkWell(
          ...
          child: Icon(idx <= bintangSaatIni? Icons.star : Icons.star_border, size: 22, color: Colors.amber.shade700),
        );
      }),
    );
  }
                  ...
                  child: Column(children: [
                    ...
                    Card(
                      ...
                      child: Padding(
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          const Divider(),
                          if (full.isNotEmpty)...[
                            const Text("✅ FULL 1 HARI:", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.green)),
                           ...full.map((a) {
                              final kar = allKar.where((k) => k.id == a.karyawanId).firstOrNull;
                              return ListTile(
                                dense: true,
                                title: Text("ID:${a.karyawanId} ${kar?.nama?? ''}"),
                                subtitle: Text(a.keterangan),
                                trailing: PopupMenuButton<String>(
                                  onSelected: (v) async => await db.updateTipeAbsen(a.id, v),
                                  itemBuilder: (_) => [const PopupMenuItem(value: 'SETENGAH', child: Text("Ubah jadi ½ Hari")), const PopupMenuItem(value: 'FULL', child: Text("Tetap Full"))],
                          ...
                          if (setengah.isNotEmpty)...[
                            const Text("🟡 ½ HARI:", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.orange)),
                           ...setengah.map((a) {
                              final kar = allKar.where((k) => k.id == a.karyawanId).firstOrNull;
                              return ListTile(
                                dense: true,
                                title: Text("ID:${a.karyawanId} ${kar?.nama?? ''} - ${a.keterangan}"),
                                trailing: PopupMenuButton<String>(
                                  onSelected: (v) async => await db.updateTipeAbsen(a.id, v),
                                  itemBuilder: (_) => [const PopupMenuItem(value: 'FULL', child: Text("Ubah jadi FULL")), const PopupMenuItem(value: 'SETENGAH', child: Text("Tetap ½"))],
                                  child: const Icon(Icons.more_vert),
                                ),
                              );
                            })
                          ...
                          if (bolong.isNotEmpty)...[
                            const Text("❌ BOLONG - GAK GAJIAN:", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red)),
                           ...bolong.map((k) => ListTile(dense: true, title: Text("ID:${k.id} ${k.nama}"), subtitle: const Text("Belum absen hari ini")))
                          ],
                        ]),
                      ),
                    ),
                    ...
                    Card(
                      ...
                      child: Padding(
                        padding: const EdgeInsets.all(12),
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          const Text("⭐ BINTANG PRO-AKTIF HARI INI", style: TextStyle(fontWeight: FontWeight.bold)),
                          const Divider(),
                         ...allKar.map((k) {
                            ...
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              ...
                              FutureBuilder<int>(
                                ...
                                builder: (c, gajiSnap) {
                                  ...
                                  final penjualan = _parse(penjualanC.text);
                                  final bebanOps = _parse(bebanOpsC.text);
                                  final tambahan = _parse(tambahanC.text);
                                  final totalMasuk = penjualan + tambahan;
                                  final totalBeban = bebanOps + gajiHari;
                                  final kotor = penjualan - bebanOps;
                                  final bersih = totalMasuk - totalBeban;
                                  return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                    Container(
                                      ...
                                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                        ...
                                        Text("Gaji Hari Ini: Rp ${fmt.format(gajiHari)}"),
                                        Text("Laba Kotor: Rp ${fmt.format(kotor)}"),
                                    ...
                                    SizedBox(
                                      ...
                                      child: ElevatedButton.icon(
                                        ...
                                        label: const Text("SIMPAN LOG LIVE HARI INI"),
        ...
        child: SingleChildScrollView(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
            ...
            if (laporan!= null)...[
              ...
              const Text("Belum ada laporan penjualan hari itu"),
          ]),
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      length: 3,
      child: Scaffold(
        appBar: AppBar(title: const Text("DASHBOARD"), backgroundColor: Colors.brown.shade800, foregroundColor: Colors.white, bottom: const TabBar(tabs: [Tab(text: "Live"), Tab(text: "Log"), Tab(text: "Absensi")])),
        body: TabBarView(children: [_liveTab(), const Center(child: Text("Log pakai filter Laba")), _kalenderTab()]),
      ),
    );
  }
}