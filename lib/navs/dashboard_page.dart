...
class _DashboardPageState extends State<DashboardPage> {
  ...
  @override void initState(){
    super.initState();
    timer = Timer.periodic(const Duration(seconds:1), (_){ if(mounted) setState(()=> nowLive = DateTime.now()); });
    _loadBulan();
  }
  ...
  Future<void> _loadBulan() async {
    ...
    }catch(e){ debugPrint("loadBulan $e"); }
  }
  ...
  Widget _liveTab(){
    ...
    return StreamBuilder<List<AbsensiData>>(
      ...
      builder: (c, absSnap){
        if(absSnap.hasError) return Center(child: Text("Error absen: ${absSnap.error}"));
        final absHariIni = absSnap.data?? [];
        return StreamBuilder<List<KaryawanData>>(
          stream: db.watchKaryawan(),
          builder: (c, karSnap){
            if(karSnap.hasError) return Center(child: Text("Error karyawan: ${karSnap.error}"));
              ...
              builder: (c, bintangSnap){
                ...
                return SingleChildScrollView(padding: const EdgeInsets.all(12), child: Column(children: [
                  ...
                  Card(color: Colors.amber.shade50, child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Text("⭐ BINTANG PRO-AKTIF HARI INI", style: TextStyle(fontWeight: FontWeight.bold)), const Divider(),
                    ...allKar.map((k){ final b=bintangMap[k.id]??0; return ListTile(dense:true, title: Text("ID:${k.id} ${k.nama}"), trailing: _bintangWidget(k.id, b), subtitle: Text(b==0?"Belum dinilai":"$b bintang")); }),
                  ]))),
                  const SizedBox(height:12),
                      ...
                      FutureBuilder<int>(future: db.hitungGajiHariIni(today), builder: (c,gajiSnap){
                        ...
                        return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Container(width: double.infinity, padding: const EdgeInsets.all(12), color: Colors.white, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Text("ESTIMASI LABA HARI INI", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.brown.shade800)),
                            Text("Gaji Hari Ini: Rp ${fmt.format(gajiHari)}"), Text("Laba Kotor: Rp ${fmt.format(kotor)}"),
                            Text("Laba Bersih: Rp ${fmt.format(bersih)}", style: TextStyle(fontWeight: FontWeight.bold, color: bersih>=0?Colors.green:Colors.red)),
                          ])),
                          const SizedBox(height:8),
                          SizedBox(width: double.infinity, child: ElevatedButton.icon(icon: const Icon(Icons.save), label: const Text("SIMPAN LOG LIVE HARI INI"), style: ElevatedButton.styleFrom(backgroundColor: Colors.brown.shade800, foregroundColor: Colors.white), onPressed: () async {
                            await db.simpanLaporanHarian(today, penjualan, bebanOps, tambahan, ketTambahanC.text, gajiHari, kotor, bersih);
                            if(context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Log Live Disimpan")));
                            _loadBulan();
                          })),
                        ]);
                      }),
  ...
  void _klikTanggal(DateTime tgl) async {
    ...
    showModalBottomSheet(context: context, isScrollControlled:true, builder: (_)=> Container(padding: const EdgeInsets.all(16), child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
      ...
      const Divider(),
      if(laporan!=null)...[Text("Penjualan: Rp ${fmt.format(laporan.totalPenjualan)}"), Text("Beban Ops: Rp ${fmt.format(laporan.bebanOperasional)}"), Text("Tambahan: Rp ${fmt.format(laporan.tambahanLain)} - ${laporan.keteranganTambahan}"), Text("Gaji Hari: Rp ${fmt.format(laporan.totalGajiHariIni)}", style: const TextStyle(fontWeight: FontWeight.bold)), Text("Laba Bersih: Rp ${fmt.format(laporan.labaBersih)}", style: TextStyle(fontWeight: FontWeight.bold, color: laporan.labaBersih>=0?Colors.green:Colors.red)))] else const Text("Belum ada laporan penjualan hari itu"),
    ]))));
  }
  ...
  Widget _logTab(){
    return StreamBuilder<List<LaporanHarianData>>(stream: (db.select(db.laporanHarian)..orderBy([(t)=> t.tanggal.desc])).watch(), builder: (c,snap){
      if(!snap.hasData) return const Center(child: CircularProgressIndicator());
      if(snap.data!.isEmpty) return const Center(child: Text("Belum ada Log Live"));
      return ListView.builder(itemCount: snap.data!.length, itemBuilder: (_,i){ final l=snap.data![i]; return Card(child: ListTile(title: Text(DateFormat('dd MMM yyyy').format(l.tanggal)), subtitle: Text("Penjualan Rp ${fmt.format(l.totalPenjualan)} | Bersih Rp ${fmt.format(l.labaBersih)}"), trailing: Text("${fmt.format(l.totalGajiHariIni)} gaji"))); });
    });
  }

  @override Widget build(BuildContext context){
    // HAPUS Scaffold - ini penyebab blank di Main
