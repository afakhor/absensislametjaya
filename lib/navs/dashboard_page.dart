    ...
    return Row(
      ...
      children: List.generate(5, (i) {
        ...
        return InkWell(
          ...
          child: Icon(idx <= bintangSaatIni ? Icons.star : Icons.star_border, size: 20, color: Colors.amber.shade700),
        );
      }),
    );
  }
                  ...
                  child: Column(
                    children: [
                      Container(
                        ...
                        child: Text("${DateFormat('EEEE, dd MMM yyyy HH:mm:ss', 'id_ID').format(nowLive)} WIB - LIVE", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                      ),
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const Divider(),
                              if (full.isNotEmpty) ...[
                                const Text("✅ FULL:", style: TextStyle(color: Colors.green, fontWeight: FontWeight.bold)),
                                ...full.map((a) {
                              ...
                              if (bolong.isNotEmpty) ...[
                                const Text("❌ BOLONG:", style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
                                ...bolong.map((k) => ListTile(dense: true, title: Text(k.nama), subtitle: const Text("Belum absen"))),
                              ],
                            ],
                          ),
                        ),
                      ...
                      Card(
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            children: [
                              const Text("⭐ BINTANG HARI INI", style: TextStyle(fontWeight: FontWeight.bold)),
                              ...allKar.map((k) => ListTile(dense: true, title: Text(k.nama), trailing: _bintangWidget(k.id, mapB[k.id] ?? 0))),
                            ],
                          ),
                        ),
                      ),
                                        ...
                                        children: [
                                          ...
                                          SizedBox(
                                            ...
                                            child: ElevatedButton(
                                              onPressed: () async {
                                                await db.simpanLaporanHarian(today, jual, beban, _parse(tambahanC.text), ketTambahanC.text, gajiHari, jual - beban, bersih);
                                                _loadBulan();
                                                if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Log Disimpan")));
                                              },
        ...
        Expanded(
          child: StreamBuilder<List<LaporanHarianData>>(
            ...
            builder: (c, snap) {
              ...
              final filtered = snap.data!.where((l) => l.tanggal.isAfter(start.subtract(const Duration(seconds: 1))) && l.tanggal.isBefore(end.add(const Duration(seconds: 1)))).toList();
                ...
                itemBuilder: (_, i) {
                  ...
                  return Card(
                    child: ListTile(
                      ...
                      subtitle: Text("Jual ${fmt.format(l.totalPenjualan)} | Bersih ${fmt.format(l.labaBersih)} | Gaji ${fmt.format(l.totalGajiHariIni)}"),
                    ),
                  );
                },
              );
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Padding(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text("LOG ${DateFormat('dd MMM yyyy').format(tgl)}", style: const TextStyle(fontWeight: FontWeight.bold)),
            const Divider(),
            ...
            if (laporan != null) ...[
              ...
              Text("Bersih Rp ${fmt.format(laporan.labaBersih)}", style: TextStyle(color: laporan.labaBersih >= 0 ? Colors.green : Colors.red, fontWeight: FontWeight.bold)),
            ] else
        ...
        Expanded(
          child: GridView.builder(
            ...
            itemBuilder: (_, i) {
              ...
              final warna = list.isEmpty ? Colors.red.shade200 : (allKaryawan.isNotEmpty && list.length >= allKaryawan.length ? Colors.green.shade200 : Colors.yellow.shade200);
