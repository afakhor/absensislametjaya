...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Widget _bintangWidget(int karyawanId, int bintangSaatIni){
    return Row(mainAxisSize: MainAxisSize.min, children: List.generate(5, (i){
      final idx=i+1;
      return InkWell(onTap: () async => await db.setBintang(karyawanId, DateTime.now(), idx), child: Icon(idx<=bintangSaatIni? Icons.star : Icons.star_border, size:20, color: Colors.amber.shade700));
    }));
  }
  ...
  Widget _liveTab(){
    ...
    return StreamBuilder<List<AbsensiData>>(stream: db.watchAbsensiHari(today), builder: (c,absSnap){
      ...
      return StreamBuilder<List<KaryawanData>>(stream: db.watchKaryawan(), builder: (c,karSnap){
        ...
        if(allKar.isEmpty) return const Center(child: Text("Belum ada Karyawan\nTambah di menu Owner dulu"));
        ...
        return StreamBuilder<List<BintangHarianData>>(stream: db.watchBintangHari(today), builder: (c,bintangSnap){
          ...
          return SingleChildScrollView(padding: const EdgeInsets.all(12), child: Column(children: [
            Container(width: double.infinity, padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.brown.shade800, borderRadius: BorderRadius.circular(12)), child: Text("${DateFormat('EEEE, dd MMM yyyy HH:mm:ss','id_ID').format(nowLive)} WIB - LIVE", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
            const SizedBox(height:12),
            Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text("HADIR ${absHariIni.length}/${allKar.length} | BOLONG ${bolong.length}"), const Divider(),
              if(full.isNotEmpty)...[const Text("✅ FULL:", style: TextStyle(color: Colors.green, fontWeight: FontWeight.bold)),...full.map((a){ final kar=allKar.firstWhere((k)=>k.id==a.karyawanId, orElse: ()=> allKar.first); return ListTile(dense:true, title: Text(kar.nama), subtitle: Text(a.keterangan), trailing: PopupMenuButton(onSelected: (v)=> db.updateTipeAbsen(a.id, v.toString()), itemBuilder: (_)=>[const PopupMenuItem(value:'SETENGAH', child: Text("Jadi ½")), const PopupMenuItem(value:'FULL', child: Text("Tetap Full"))])); })],
              if(setengah.isNotEmpty)...[const Text("🟡 ½ HARI:", style: TextStyle(color: Colors.orange, fontWeight: FontWeight.bold)),...setengah.map((a){ final kar=allKar.firstWhere((k)=>k.id==a.karyawanId, orElse: ()=> allKar.first); return ListTile(dense:true, title: Text(kar.nama), trailing: PopupMenuButton(onSelected: (v)=> db.updateTipeAbsen(a.id, v.toString()), itemBuilder: (_)=>[const PopupMenuItem(value:'FULL', child: Text("Jadi FULL"))])); })],
              if(bolong.isNotEmpty)...[const Text("❌ BOLONG:", style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),...bolong.map((k)=> ListTile(dense:true, title: Text(k.nama), subtitle: const Text("Belum absen")))],
            ]))),
            Card(color: Colors.amber.shade50, child: Padding(padding: const EdgeInsets.all(12), child: Column(children: [const Text("⭐ BINTANG HARI INI", style: TextStyle(fontWeight: FontWeight.bold)),...allKar.map((k)=> ListTile(dense:true, title: Text(k.nama), trailing: _bintangWidget(k.id, mapB[k.id]??0)))]))),
            StreamBuilder<LaporanHarianData?>(stream: db.watchLaporanHari(today), builder: (c,lapSnap){
              ...
              return Card(color: Colors.blue.shade50, child: Padding(padding: const EdgeInsets.all(12), child: Column(children: [
                TextField(controller: penjualanC, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText:"Penjualan Rp")),
                TextField(controller: bebanOpsC, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText:"Beban Ops Rp")),
                TextField(controller: tambahanC, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText:"Tambahan Rp")),
                TextField(controller: ketTambahanC, decoration: const InputDecoration(labelText:"Keterangan")),
                const SizedBox(height:8),
                FutureBuilder<int>(future: db.hitungGajiHariIni(today), builder: (c,gajiSnap){
                  final gajiHari=gajiSnap.data??0; final jual=_parse(penjualanC.text); final beban=_parse(bebanOpsC.text); final bersih=jual - beban - gajiHari + _parse(tambahanC.text);
                  return Column(children: [
                    Text("Gaji Hari Ini Rp ${fmt.format(gajiHari)} | Bersih Rp ${fmt.format(bersih)}", style: TextStyle(fontWeight: FontWeight.bold, color: bersih>=0?Colors.green:Colors.red)),
                    SizedBox(width: double.infinity, child: ElevatedButton(onPressed: () async { await db.simpanLaporanHarian(today, jual, beban, _parse(tambahanC.text), ketTambahanC.text, gajiHari, jual-beban, bersih); _loadBulan(); if(context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Log Disimpan"))); }, child: const Text("SIMPAN LOG LIVE")))
                  ]);
                }),
              ])));
            }),
  ...
  Widget _logTab(){
    return Column(children: [
      Container(color: Colors.brown.shade50, padding: const EdgeInsets.all(8), child: Row(children: [
        Expanded(child: TextButton(onPressed: () async { final p=await showDatePicker(context: context, initialDate: logMulai, firstDate: DateTime(2023), lastDate: DateTime.now()); if(p!=null) setState(()=> logMulai=p); }, child: Text("Mulai ${DateFormat('dd/MM/yy').format(logMulai)}"))),
        Expanded(child: TextButton(onPressed: () async { final p=await showDatePicker(context: context, initialDate: logSelesai, firstDate: DateTime(2023), lastDate: DateTime.now()); if(p!=null) setState(()=> logSelesai=p); }, child: Text("Sampai ${DateFormat('dd/MM/yy').format(logSelesai)}"))),
      ])),
      Expanded(child: StreamBuilder<List<LaporanHarianData>>(stream: (db.select(db.laporanHarian)..orderBy([(t)=> OrderingTerm.desc(t.tanggal)])).watch(), builder: (c,snap){
        if(!snap.hasData) return const Center(child: CircularProgressIndicator());
        final filtered=snap.data!.where((l)=> l.tanggal.isAfter(logMulai.subtract(const Duration(days:1))) && l.tanggal.isBefore(logSelesai.add(const Duration(days:1)))).toList();
        if(filtered.isEmpty) return const Center(child: Text("Belum ada log di range ini"));
        return ListView.builder(itemCount: filtered.length, itemBuilder: (_,i){ final l=filtered[i]; return Card(child: ListTile(title: Text(DateFormat('dd MMM yyyy').format(l.tanggal)), subtitle: Text("Jual ${fmt.format(l.totalPenjualan)} | Bersih ${fmt.format(l.labaBersih)} | Gaji ${fmt.format(l.totalGajiHariIni)}"))); });
      })),
    ]);
  }
  ...
  void _klikTanggal(DateTime tgl) async {
    ...
    showModalBottomSheet(context: context, isScrollControlled:true, builder: (_)=> Padding(padding: const EdgeInsets.all(16), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text("LOG ${DateFormat('dd MMM yyyy').format(tgl)}", style: const TextStyle(fontWeight: FontWeight.bold)), const Divider(),
      Text("Masuk ${absen.length}/${allKaryawan.length} | Bolong ${bolong.length}"),
      if(bolong.isNotEmpty) Text("❌ ${bolong.map((e)=>e.nama).join(', ')}"),
      Text("⭐ ${bintang.isEmpty?"Belum ada": bintang.map((b)=> "${b.karyawanId}:${b.bintang}").join(', ')}"),
      if(laporan!=null)...[Text("Jual Rp ${fmt.format(laporan.totalPenjualan)}"), Text("Bersih Rp ${fmt.format(laporan.labaBersih)}", style: TextStyle(color: laporan.labaBersih>=0?Colors.green:Colors.red, fontWeight: FontWeight.bold))] else const Text("Belum ada laporan"),
    ])));
  }
  ...
  Widget _kalenderTab(){
    ...
    return Column(children: [
      ...
      Expanded(child: GridView.builder(padding: const EdgeInsets.all(8), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount:7, childAspectRatio:0.9), itemCount: startWeekday+daysInMonth, itemBuilder: (_,i){
        ...
        final day=i-startWeekday+1; final tgl=DateTime(bulan.year, bulan.month, day);
        ...
        return GestureDetector(onTap: ()=> _klikTanggal(tgl), child: Container(margin: const EdgeInsets.all(3), decoration: BoxDecoration(color: warna, borderRadius: BorderRadius.circular(8)), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Text("$day", style: const TextStyle(fontWeight: FontWeight.bold)), Text("${list.length}/${allKaryawan.length}", style: const TextStyle(fontSize:9))])));
      })),
    ]);
  }
